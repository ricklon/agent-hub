...
def make_router(
    ...
    async def page_voice_session(websocket: WebSocket) -> None:
        ...
        async def _speak_chunk(
            text: str, *, first: bool, interrupt: asyncio.Event, voice: Persona
        ) -> int:
            """Voice one sentence. Returns ms spent synthesizing (0 if the page voices it)."""
            if voice_mode == "hub":
                # Synthesize before announcing, so the start message carries
                # any voice fallback notice for this reply.
        ...
        async def _run_turn(transcript: str, interrupt: asyncio.Event) -> None:
            ...
            async with pipeline_lock:
                ...
                # Re-read so a voice changed mid-session applies to this reply.
                voice = await store.get_persona_for_device(device_id) or persona
        ...
        try:
            while True:
                ...
                if "bytes" in msg:
                    ...
                    if ctrl.get("type") == "wake_word":
                        ...
                        detector = load_detector(config, wake_word or None)
                        # Barge-in needs the model: it hears its name over the
                        # reply's echo, so the page keeps the mic open while
                        # speaking only then. Transcript matching could be
                        # triggered by that echo, so the page mutes instead.
                        await websocket.send_text(
                            json.dumps({"type": "wake_mode", "model": detector is not None})
                        )
                        logger.bind(tag=_TAG).info(
            ...
            # A reply still in flight has no one to speak to.
            if active_turn is not None and not active_turn.done():
                active_turn.cancel()
